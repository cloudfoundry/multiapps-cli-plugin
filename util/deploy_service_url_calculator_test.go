...
import (
	...
	"github.com/cloudfoundry-incubator/multiapps-cli-plugin/util/fakes"
	...
	Describe("ComputeDeployServiceURL", func() {
		...
		Context("when a command line option for the url is present and there is a shared domain", func() {
			It("should return the value of the command line option", func() {
				...
				cmdOptValue := "https://deploy-service.test2.ondemand.com"
				deployServiceURLCalculator := util.NewDeployServiceURLCalculatorWithHttpExecutor(cfrestclient_fakes.NewFakeCloudFoundryClient(domains, nil), fakeHttpExecutor)
				Expect(deployServiceURLCalculator.ComputeDeployServiceURL(cmdOptValue)).To(Equal(cmdOptValue))
			})
		})
	})
})
